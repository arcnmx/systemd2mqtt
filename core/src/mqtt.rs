use {
	...
	hass_mqtt_client::{HassMqttClient, HassMqttOptions, Message},
	log::{error, warn},
	std::{
		borrow::Cow,
		collections::HashMap,
	...
	systemd2mqtt_discovery::{
		ConfiguredEntity as _, ConfiguredUnit, DiagButton, EntityContext, EntityTopics, MqttTopic, QOS,
	},
	systemd2mqtt_payload::{Error, Result, SerializeExt as _, ServiceCommand, ServiceStatus},
	zbus_systemd::systemd1::ManagerProxy,
};

pub struct MqttConnection<'c> {
